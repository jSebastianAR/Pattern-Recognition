...
%{
function [GROUPS] = getAllPositions(GROUPS,plottedPoints)%,imagen)
%[rows,cols,~] = size(imagen)
%pause
numGroups = length(GROUPS);
    ...
    for i=1:numGroups
        ...
        for j=1:elements
            if elements>100
                ejex = randi([plottedPoints(i,1)-100 plottedPoints(i,1)+100]);
                ejey = randi([plottedPoints(i,2)-100 plottedPoints(i,2)+100]);
            else
                ejex = randi([plottedPoints(i,1)-elements plottedPoints(i,1)+elements]);
                ejey = randi([plottedPoints(i,2)-elements plottedPoints(i,2)+elements]);
            end
            fprintf('\nplot in [%d,%d]\n',ejex,ejey)
            plotAllGroups(ejex,ejey,i)
            GROUPS(i).plotPoints = [GROUPS(i).plotPoints;ejex,ejey];
