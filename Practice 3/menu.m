...
inverseE = []
while(opcion~=0)
    opcion = input('Que clasificador deseas usar? 0.-Salir 1-Euclidiano 2.-Mahalanobis 3.-Bayesiano ');
    ...
    if opcion==1
        ...
        numClase = clasePerteneciente(distanciasM,opcion);
    ...
    elseif opcion==2
        ...
        matricesE
        distances
        numClass = clasePerteneciente(distances,opcion);
    ...
    elseif opcion==3
        ...
        elementosVector = numel(vectorUsuario);
        if elementosVector~=0 %if there is an old vector plotted we need to erase it
            plot(vectorUsuario(1),vectorUsuario(2),'vw','MarkerFaceColor','w','MarkerSize',5)%borra el punto
            vectorUsuario = [];
        end
        ...
        vectory = input('Ingresa el valor de la coordenada en y ');
        vectorUsuario = [vectorx vectory]
        plot(vectorUsuario(1),vectorUsuario(2),'vm','MarkerFaceColor','m','MarkerSize',5);
        ...
        matricesE
        distances = Mahalanobis(matricesE,MEDIAS,vectorUsuario,numFilas)
        distances = (1/100)*(distances)
        shape = size(matricesE);
        numFilasE = shape(1);
        
        [P,PN] = Bayesiano(matricesE,distances,numFilasE);
        PN
